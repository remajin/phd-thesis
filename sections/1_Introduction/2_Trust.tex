...
synchronous fibers or an asynchronous programming style.  
%While \trust{} provides some value in isolation, its full power is only apparent when combined with a runtime, providing additional concurrency, request scheduling and more. Now, depending on the current context as well as compile-time settings, the call to {\it apply()} may result in a very wide range of outcomes including (a) accessing the property directly, (b) acquiring a lock, then accessing the property, (c) sending a delegation request to a different thread, which in turn accesses the property, and/or (d) suspending the current thread and resuming another. 
