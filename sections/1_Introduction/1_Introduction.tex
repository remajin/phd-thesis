Delegation is a concurrency technique where a thread hands off work on a shared object to a single designated 
owner instead of accessing the object directly. Because only the owner thread ever mutates the object, no locks 
are needed, other threads interact with it by sending requests. This avoids data races, simplifies reasoning, 
and can outperform lock-based designs by eliminating contention and complex synchronization.
As delegation does not rely on shared memory, its applications extend naturally beyond local concurrency to 
broader distributed settings. We present two system, \trust{} and
