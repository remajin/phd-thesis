...
Shared memory enables programmers to write multi-threaded applications, making them perfrom better but also increase 
...
of the shared objects, which also comlicates the synchronization mechanisms. Disributed applications, historically, 
...
few changes and the applications have the illusion of running on the same machine. Most of the distribution of 
work is handled behind the scenes, but programmers have the option to customize where the worker threads should 
run and where data should be held in the rack if they so choose. This can be acheived by mapping the server memory 
