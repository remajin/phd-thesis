...
A flag in the request slot indicates that new requests are ready to be processed. See Section~\ref{st:slots} for details on request and response slot structure.
...
\begin{figure*}[t]
  ...
  The response slot contains a matching {\tt ready} bit, as well as one (fixed, variable, or zero-sized) response per request in the matching request slot. 
...
Serialized closure arguments are appended next, followed by the next request. 
...
the property is also dropped and the associated memory is freed so as not to have memory leakage. To achieve 
that trusts need to be reference counted, but a na$\ddot{\i}$ve integer count will not suffice due to a combination of 
...
\begin{itemize}
	...
	delegation would cause the system to hang and never finish. In the implementation, a runtime assertion 
	raises a panic if this condition is encountered.
	\item If the integer used for counting references, \textit{refcount} below, is incremented and decremented 
	asynchronously, i.e. with a nonblocking delegation call, it could lead to use after free bugs. Consider the 
	following scenario: Thread A clones a trust with only one reference and sends an async request 
...
a trust from within a delegated context, which can be quite restrictive. To get around that, \trust{} uses a new 
