...
code. Figure~\ref{fig:design} shows a high level view of what a rack with multiple \name{} applications looks like. 
...
fibers per system thread for concurrent operation as shown in Figure~\ref{fig:process}. Fibers are lightweight, 
...
The Trust can be used to delegate any work that needs access to this property. Figure~\ref{fig:trustntrustee} shows the 
...
\subsection{Pointers and Serialized Data}
\label{sg:noref}
...
closures to comply with the NoRef trait as shown in Figure~\ref{fig:trustntrustee}. This is implemented using the code 
shown in Table~\ref{tab:auto}. This however limits severely what 
...
serialized along with the closure as an extra parameter to the delegated closure. Table~\ref{tab:noref} shows a code 
example where a Box, a Rust smart pointer, is serialized instead of being captured. If the programmer makes a 
mistake and tries to use a closure that has captured a reference for delegation, a custom error message at 
compile time will inform them what the issue is and direct them to use the serialization method instead. An example 
of this message is shown in Figure~\ref{fig:error}.
...
Figure~\ref{fig:req_slot} shows the format for both the request and response slots.\\
...
Instead, parts of it are allocated on each machine. Figure~\ref{fig:req_mem_layout} shows an example layout for a system 
...
in trustee memory \textit{square} and so on. Figure~\ref{fig:req_mem_layout} also shows a few additional examples. There 
is a similar system in place for response slots as well.

\subsection{Scaling impact of increasing number of Queue Pairs}

\input{figures/qp_scaling}
...
$num\_machines^2 \times num\_threads\_per\_machine$. Figure~\ref{fig:qp_scaling} shows the reduction in the number of 
connections gained by using this design. Figure~\ref{fig:qp_scaling_6m} shows an experiment that demonstrates 
