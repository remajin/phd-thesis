...
Figure~\ref{fig:echo_tput} shows the results of our experiment. Here we use 16 threads per machine as the eRPC benchmark
...
Figure~\ref{fig:echo_reqsize} shows the performance of all three systems across a range of request sizes. The MPI system 
...
Figure~\ref{fig:erpc_gsm1} shows the results of this experiment. Each data point here represents a doubling of batch 
...
to ensure maximum performance. The key space in
...
Figure~\ref{fig:kv_ycsbc_u} and Figure~\ref{fig:kv_ycsbb_u} show the number of operations per second for a read only workload and 
...
Figure~\ref{fig:kv_ycsbc_z} and Figure~\ref{fig:kv_ycsbb_z} show the number of operations per second for a read only workload and 
...
Figure~\ref{fig:sssp_threads_n27} shows the results of SSSP on a graph with $2^{27}$ nodes and average degree of 32, 
...
its performance while remaining a general-purpose framework. Similarly, Figure~\ref{fig:sssp_size} shows \name{} 
