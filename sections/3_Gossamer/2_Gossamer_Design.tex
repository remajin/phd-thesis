...
discover the rack size and store metadat like gossamer-id, machine-ip etc. The daemon is responsible for 
...
userspace threads that can spawn on any system thread that is part of the \name{} process, and on any machine 
...
severly as TCP is much slower than RDMA. We use TCP here instead of RDMA because, as mentioned in \ref{s:rdmaapi}, 
...
all connectedto switch with 100Gb/s ethernet links. Each thread writes 256 bytes in a randomly chosen remote 
thread's memory twenty million times. As the number of threads increases, thereby increasing the number of QPs, 
the system with reduced number of QPs maintains peak throughput where as the system with quadratic number of QPs 
