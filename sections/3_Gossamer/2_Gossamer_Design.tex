...
Similarly, the responses for all of the requests in a request slot are batched and written to the client's 
memory with a single RDMA write. Unlike a request however, the size of a response is not always known whe 
submitting a request. While the known size of requests allows us to pack as many requests in a request slot
as would fit, it can lead to situations where not all of the responses will fit in a single response slot. 
Based on this observation, the responses are divided in two types.

\begin{itemize}
	\item Statically known sized responses. Reffered to as fixed size response here.
	\item Dynamically sized responses.
\end{itemize}

Both of these can be further divided into responses that are small and can fit inside the slot or large and
cannot fit inside the slot. For fixed size and small responses, the response can be sent inline without any
metadata. For dynamically sized small responses, the size of the response is accompanied with them. For large
responses, both fixed an dynamically sized, the response is sent using TCP and the pointer to the remote 
buffer is sent in the response slot. In addition, if the response size is zero and statically known, then 
nothing is sent in the slot. \ref{fig:req_slot} shows the format for both the request and response 
slots.\\
...
is a bit that indicates if there is a single large reequest in the slot or muliptle small ones. As discussed 
in \ref{tcp_fallback}, if the request will not fit in the slot it is sent using TCP and only the relevant 
...
$(num\_machines*num\_threads\_per\_machine)^2$. This also resluts in lost performance due to NIC cache 
...
$num\_machines^2*num\_threads\_per\_machine$. \ref{fig:qp_scaling_6m} shows an experiment that demonstrates the performance 
