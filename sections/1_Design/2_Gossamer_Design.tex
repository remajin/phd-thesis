...
being sent with each request. As an example if each request request captures 32 bytes of data, then along with 
8 bytes for request header, total request size is 40 bytes. Assuming 1kB requestSlot, one batch can have at 
most 25 requests. While this allows us to optimize for small requests it also places a hard limit on how much 
data a request can capture i.e. the size of the requestSlot. To work aound this limit and support larger 
requests, \name{} uses TCP to send any captured data larger than that separately while the request header goes 
in the requestSlot as normal. This degrades the performance severly as TCP is much slower than RDMA. We use 
TCP here instead of RDMA because, as mentioned in \ref{s:rdmaapi}, RDMA needs the memory used to be 
pre-registered with the NIC. This would also place an upper limit on how much data can be sent using RDMA at 
once. Since there would be an upper limit anyway and larger requests are not the focus for \name{}, we decided 
to opt for the simpler design in place at the moment.

\subsection{Request Format}
\label{s:header}

\subsection{Distributed Refcount}